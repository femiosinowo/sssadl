...
public partial class Templates_Content : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
      ...
      //  mainContent.Text = "<!-- -->";
        ...
        if (!Page.IsPostBack)
        {
            ...
            contactForm.DefaultFormID = long.Parse(formID);
           ...
           // int i = 1;
            contactSelect.Items.Add(new ListItem("- Select a subject -", defaultFormID));


            selectOutput = "<option selected value='" + defaultFormID + "'>- Select a subject -</option>";
            Dictionary<long, string> contenIdTitles = commonfunctions.getCollectionContentIds(14);
            foreach (var a in contenIdTitles)
            {
                ListItem li = new ListItem();
                li.Text = a.Value;
                li.Value = a.Key.ToString();
    ...
    protected void changeDisplayForm(object sender, EventArgs e)
    {
        //stuff that never gets hit
      //  contactForm.DefaultFormID = long.Parse(contactSelect.SelectedValue);
      //  contactForm.Page = Page;
     //   contactForm.Fill();
       // Response.Write(contactForm.EkItem.Html);
        //Response.Write(contactSelect.SelectedValue);
        Response.Redirect("http://dev.ssadl.local/about/contact.aspx?formid=" + contactSelect.SelectedValue);
    }
