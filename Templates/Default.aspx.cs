...
public partial class Templates_Default : System.Web.UI.Page
{
            ...
            //{
                ...
                if (searchLinksCId > 0)
                {
                   ...
                   // MainBackgroundImageALT = commonfunctions.getFieldAttributeValue(contentXML, "MainBackgroundImage", "img", "alt", "/root");
                    try
                    {
                        ArrayList BggImgAr = new ArrayList();
                        XmlNodeList xListBggImages = contentXML.SelectNodes("/root/BggImages");
                        foreach (XmlElement xnBggImg in xListBggImages)
                        {
                            BggImgAr.Add(xnBggImg.InnerXml);

                        }
                        Random r = new Random();
                        int randomNumber = r.Next(0, BggImgAr.Count);
                        // Response.Write(BggImgAr[randomNumber]);
                        string displayImage = BggImgAr[randomNumber].ToString();
                        //img bgg
                        string matchSRC = Regex.Match(displayImage, "<img.+?src=[\"'](.+?)[\"'].+?>", RegexOptions.IgnoreCase).Groups[1].Value;
                        if (matchSRC != string.Empty)
                        {
                            homeContent.Attributes.Remove("style");
                            homeContent.Attributes.Add("style", "background:url('" + matchSRC + "') no-repeat scroll center top #ffffff");
                        }
                        //Response.Write(BggImgAr[randomNumber]);
                    }
                    catch
                    {
                        //home page has no background images attached to the smartform in ektron workarea
                    }
    ...
    private void generateMenu(string menuXML)
    {
      ...
      //  Response.Write(menuXML);
        ...
        foreach (XmlElement xnItem in xnMenuData)
        {
            ...
            if (ItemType == "content" || ItemType == "ExternalLink")
            {
    ...
    private void GetLatestNews(int count = 4)
    {
        ...
        lvLatestNews.DataSource = DataBase.dbDataTable("SELECT Top " + count.ToString() + " * FROM [ViewAlllNews_SSADL] order by Date DESC" , "Ektron.Dbconnection");
        lvLatestNews.DataBind();
    }
    protected void latestNewsBinding(object sender, ListViewItemEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            Literal link = (Literal)ditem.FindControl("link");

            string contentid = item["content_id"].ToString();
            link.Text = commonfunctions.getQuickLink(long.Parse(contentid));
