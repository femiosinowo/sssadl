...
public partial class admin_reports_Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string toDatePrior = DateTime.Now.AddYears(-1).ToString("d");

        string sql = " select count(DISTINCT  ClickedByPIN) as uniqueVisitors , count(*) as NumberofHits    from [ClickTracking]  ";
        string sqlLastYearFull = sql + " where (ClickedDateTime between '" + LastYearFY_Start + "' and '" + LastYearFY_End + "') ";
        string sqlLastYeartoDate = sql + " where (ClickedDateTime between '" + LastYearFY_Start + "' and '" + toDatePrior + "') ";
        string sqlthisYeartoDate = sql + " where (ClickedDateTime between '" + thisYearFY_Start + "' and '" + toDate + "') ";
      //  Response.Write(sqlLastYearFull + "<br/>" + sqlLastYeartoDate + "<br/>" + sqlthisYeartoDate);
