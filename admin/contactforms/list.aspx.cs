...
public partial class admin_users_Default : System.Web.UI.Page
{
    ...
    {
        string whatform = string.Empty;
        whatform = Request.QueryString["form"];
        if(whatform != null) {
        switch (whatform)
        {
            case "passwordassistance":
                passwordassistance.Visible = true;
    ...
    protected void SearchResult_ItemDatabound(object sender, ListViewItemEventArgs e)
    {
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            viewLink.Text = "View";
            viewLink.NavigateUrl = "view.aspx?id=" + item["ID"].ToString().Trim() + "&keepThis=true&TB_iframe=true&height=650&width=600";
            viewLink.CssClass = "thickbox";

            editLink.Text = "Edit";
            editLink.NavigateUrl = "/edit.aspx?id=" + item["ID"].ToString().Trim() + "&keepThis=true&TB_iframe=true&height=650&width=600";
    ...
    protected void Datapager_prender(object sender, EventArgs e)
    {
        ...
        string searchString = "Select * from PasswordAssistanceForm;";
